...
def ascon_leakage_model(nonce_0, nonce_1, state_register_index, bit_index, debug=False):
    """
    ...
    Inputs:
        nonce_0 (int): The most significant half of the nonce used for the ASCON cipher, 
        in hexadecimal format, correspondig to the register x3.
        
        nonce_1 (int): The least significant half of the nonce used for the ASCON cipher, 
    ...
    # The notation "nonce_0_j" means the j-th bit of the first nonce half,
    # "nonce_0_j36" means the (j+36)-th bit of the first nonce half,
    # and "nonce_0_j45" means the (j+45)-th bit of the first nonce half.
    # The same applies to the second nonce half
    nonce_0_j     = (int(nonce_0) >> (bit_index % 64)) & 1          # Conversion to int is necessary for bitwise operations
    nonce_0_j36   = (int(nonce_0) >> ((bit_index + 36) % 64)) & 1
    nonce_0_j45   = (int(nonce_0) >> ((bit_index + 45) % 64)) & 1

    nonce_1_j     = (int(nonce_1) >> (bit_index % 64)) & 1
    nonce_1_j36   = (int(nonce_1) >> ((bit_index + 36) % 64)) & 1
    nonce_1_j45   = (int(nonce_1) >> ((bit_index + 45) % 64)) & 1
    ...
    for key_guess in range(8):
        ...
        if state_register_index == 0:
            ...
            z0_j =  (k0 & (nonce_1_j ^ 1) ^ nonce_0_j) ^ \
                    (k1 & (nonce_1_j36 ^ 1) ^ nonce_0_j36) ^ \
                    (k2 & (nonce_1_j45 ^ 1) ^ nonce_0_j45)
            
            # Append the expected value to the leakage model
            leakage_model[key_guess] = z0_j

            if debug:
                print(f"Key guess {key_guess}: z0_j = {z0_j}, " \
                      f"nonce_0_j = {nonce_0_j}, nonce_0_j36 = {nonce_0_j36}, " \
                      f"nonce_0_j45 = {nonce_0_j45}, " \
                      f"nonce_1_j = {nonce_1_j}, nonce_1_j36 = {nonce_1_j36}, " \
                      f"nonce_1_j45 = {nonce_1_j45}")
        else:
