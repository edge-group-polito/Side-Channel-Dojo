    ...
    with h5py.File(traces_file, 'r') as f_read_traces:
        ...
        if cpa_phase_full_key:
            ...
            def cpa_worker_k0(args):
                key_bit, traces_shape, nonces_shape, traces_shm_name, nonces_shm_name, initialization_vector, sbox_type, key_0_correct, partial_traces_len = args
                traces_shm = shared_memory.SharedMemory(name=traces_shm_name)
                nonces_shm = shared_memory.SharedMemory(name=nonces_shm_name)
                traces = np.ndarray(traces_shape, dtype=np.float64, buffer=traces_shm.buf, order='C')[:partial_traces_len]
                nonces = np.ndarray(nonces_shape, dtype=np.uint64, buffer=nonces_shm.buf, order='C')[:partial_traces_len]
                ...
                sorted_indices = np.argsort(-corr_vs_keyguess)
                rank = np.where(sorted_indices == correct_guess)[0][0]
                traces_shm.close()
                nonces_shm.close()
                return (key_bit, best_key_guess, rank)

            print("S0 Key Recovery Phase")
            pbar = tqdm(range(1, (traces.shape[0] // resolution) + 1), desc="Using 0 traces")
            ...
            for count in pbar:
                ...
                partial_traces_len = count*resolution
                args_list_k0 = [
            ...
            with open(f"../x-heep/Graphs/ASCON_c/ASCON_success_rate_parallel_S0_sbox_{sbox_type}.json", "w") as f:
                json.dump(success_rate_vs_traces_0[sbox_type], f)

            # Save the key rank to a JSON file
            with open(f"../x-heep/Graphs/ASCON_c/ASCON_key_ranks_parallel_S0_sbox_{sbox_type}.json", "w") as f:
                json.dump(to_python_types(key_ranks_vs_traces_0[sbox_type]), f)



            def cpa_worker_k1(args):
                key_bit, traces_shape, nonces_shape, traces_shm_name, nonces_shm_name, initialization_vector, sbox_type, k0, key_1_correct, partial_traces_len = args
                traces_shm = shared_memory.SharedMemory(name=traces_shm_name)
                nonces_shm = shared_memory.SharedMemory(name=nonces_shm_name)
                traces = np.ndarray(traces_shape, dtype=np.float64, buffer=traces_shm.buf, order='C')[:partial_traces_len]
                nonces = np.ndarray(nonces_shape, dtype=np.uint64, buffer=nonces_shm.buf, order='C')[:partial_traces_len]
            ...
            pbar = tqdm(range(1, (traces.shape[0] // resolution) + 1), desc="Using 0 traces")
            # Prepare shared memory for traces and nonces (already done above)
            for count in pbar:
                pbar.set_description(f"Using {count*resolution} traces")
                partial_traces_len = count*resolution
                args_list_k1 = [
            ...
            # Save the success rate to a JSON file
            with open(f"../x-heep/Graphs/ASCON_c/ASCON_success_rate_parallel_S1_sbox_{sbox_type}.json", "w") as f:
                json.dump(success_rate_vs_traces_1[sbox_type], f)

            # Save the key rank to a JSON file
            with open(f"../x-heep/Graphs/ASCON_c/ASCON_key_ranks_parallel_S1_sbox_{sbox_type}.json", "w") as f:
